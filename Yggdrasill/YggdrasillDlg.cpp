...
static LSTATUS SetCTRLFLAG1(HKEY regKey, bool online) {
    ...
    err = RegQueryValueEx(regKey, _T("CTRLFLAG1"), 0, &valType, (LPBYTE)&val,
                          &sz);
    if(err == ERROR_SUCCESS) {
        ...
        err = RegSetValueEx(regKey, _T("CTRLFLAG1"), 0, REG_DWORD,
                            (const BYTE*)&val, sz);
    }
...
void CYggdrasillDlg::OnPaint() {
    if (IsIconic()) {
        CPaintDC dc(this);
        WPARAM hdc = reinterpret_cast<WPARAM>(dc.GetSafeHdc());

        SendMessage(WM_ICONERASEBKGND, hdc, 0);
...
void CYggdrasillDlg::OnBnClickedPsoOptsButton() {
    ...
    if(!CreateProcess(NULL, cmd, NULL, NULL, FALSE, 0, NULL, dir.c_str(), &si,
                      &pi)) {
